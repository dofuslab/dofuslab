...
export const copyUserLinkToClipboard = async (
  data: currentUser,
  t: TFunction,
  linkTextareaRef: React.MutableRefObject<HTMLTextAreaElement | null>,
) => {
  const url = `${window.location.protocol}//${window.location.host}/user/${data?.currentUser?.username}/`;
  try {
    if (navigator.clipboard) {
      await navigator.clipboard.writeText(url);
    } else {
    ...
    notification.success({
      ...
      description: t('COPY_LINK_SUCCESS', { ns: 'common' }),
    });
  } catch (e) {
    notification.error({
      message: t('ERROR', { ns: 'common' }),
      description: t('ERROR_OCCURRED', { ns: 'common' }),
    });
  }
};
