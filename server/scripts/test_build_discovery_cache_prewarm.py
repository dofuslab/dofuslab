...
class BuildDiscoveryCachePrewarmTest(unittest.TestCase):
    def test_prewarm_query_matrix_reports_supported_profiles(self):
        ...
        self.assertEqual(report["summary"]["rowCount"], 6)
        self.assertEqual(report["summary"]["cacheHits"], 0)
        self.assertEqual(report["summary"]["cacheMisses"], 6)
        self.assertEqual(report["summary"]["emptyResults"], 0)
        self.assertEqual(report["summary"]["elapsed"]["count"], 6)
        self.assertEqual(report["summary"]["cacheHitElapsed"]["count"], 0)
        self.assertEqual(
            seen,
            [
                (("strength",), 11),
    ...
    def test_prewarm_query_matrix_fails_on_empty_results(self):
        ...
        self.assertEqual(report["summary"]["emptyResults"], 3)
    ...
    def test_elapsed_summary_reports_nearest_rank_p95(self):
        module = load_prewarm_module()

        summary = module.elapsed_summary(
            [{"elapsedMs": value} for value in [10.0, 20.0, 30.0, 40.0]]
        )
    ...
    def test_prewarm_query_matrix_can_bound_hit_p95(self):
        module = load_prewarm_module()
        ...
        self.assertEqual(report["status"], "fail")
        self.assertEqual(report["requirements"]["maxHitP95Ms"], -1)
        self.assertIn("p95 threshold", report["failures"][0])
