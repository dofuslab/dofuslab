...
GRAPHQL_QUERY = """mutation BuildDiscoveryProductionGate(
  $input: BuildDiscoveryInput!
) {
  buildDiscovery(input: $input) {
    status solverVersion cacheKey
    cache { status storage solver }
    diagnostics { solver workers solverStatus elapsedMs }
    builds { score }
  }
}"""
...
def graphql_url(base_url: str) -> str:
    return urljoin(base_url.rstrip("/") + "/", "api/graphql")
...
def target_variables(target: Any) -> dict[str, Any]:
    return {
...
def _perform(
    ...
    try:
        envelope = request_fn(
            url,
            {"query": GRAPHQL_QUERY, "variables": {"input": variables}},
            timeout,
        )
...
def run_gate(
    *,
    ...
    cold_rows = []
    variables_by_target = []
    for target in ALL_CLASS_LEVEL_200_TARGETS:
    ...
    failures = []
    complete = sum(
        str(row["status"]).lower() == "complete" and not row["errors"]
...
def main() -> None:
    ...
    report = run_gate(
        base_url=args.base_url, warm_requests=args.warm_requests,
