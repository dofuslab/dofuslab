...
from oneoff.build_discovery_spell_profiles import derive_all_spell_profiles
...
def validate_build_discovery_index(index: Any) -> None:
        ...
        for element in ELEMENT_PROFILES:
            for level in PROFILE_LEVELS:
                ...
                if not matches:
                    ...
                    errors.append(f"spell profile {label} is not hydratable")
    ...
    if errors:
        ...
        raise BuildDiscoveryIndexValidationError(
            "Build Discovery index is incomplete: "
            + "; ".join(preview)
...
def normalize_source_weapon_effect(effect: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if not effect_type:
        effect_type = SOURCE_WEAPON_EFFECT_TYPES.get(
            str(effect.get("stat", "")).lower()
        )
    if not effect_type:
        return None
...
def normalize_source_weapon_stats(
...
def normalize_source_item(item: dict[str, Any], source_filename: str) -> dict[str, Any]:
    ...
    normalized["weaponStats"] = normalize_source_weapon_stats(
        normalized.get("weaponStats")
    )
    return normalized
...
def load_source_item_records() -> list[dict[str, Any]]:
    ...
    for filename in SOURCE_ITEM_FILES:
        items.extend(
            normalize_source_item(item, filename) for item in load_source_json(filename)
        )
    return items


def load_source_sets() -> dict[str, dict[str, Any]]:
    return {str(set_obj["id"]): set_obj for set_obj in load_source_json("sets.json")}


def clear_core_data_caches(core: Any) -> None:
    for function_name in (
        "load_build_discovery_index",
        "load_all_item_records",
        "load_sets",
    ):
        cache_clear = getattr(getattr(core, function_name, None), "cache_clear", None)
        if cache_clear:
            cache_clear()


@contextmanager
def core_db_source() -> Any:
    from oneoff import build_discovery_core as core

    original_index_path = core.BUILD_DISCOVERY_INDEX_PATH
    core.BUILD_DISCOVERY_INDEX_PATH = ""
    clear_core_data_caches(core)
    try:
        yield core
    finally:
        core.BUILD_DISCOVERY_INDEX_PATH = original_index_path
        clear_core_data_caches(core)


def load_db_item_records() -> tuple[dict[str, Any], ...]:
    with core_db_source() as core:
        return core.load_all_item_records()


def load_db_sets() -> dict[str, dict[str, Any]]:
    with core_db_source() as core:
        return core.load_sets()
...
def item_flags(item: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "actionStats": {
            stat: stats.get(stat, 0) for stat in ACTION_STATS if stats.get(stat, 0)
        },
    }
...
def build_item_indexes(items: list[dict[str, Any]]) -> dict[str, Any]:
    normal_gear_by_bucket: dict[str, list[str]] = {
        bucket: [] for bucket, _, _ in LEVEL_BUCKETS
    }
    dofus_like_by_bucket: dict[str, list[str]] = {
        bucket: [] for bucket, _, _ in LEVEL_BUCKETS
    }
    ...
    return {
        "normalGearByLevelBucket": {
            bucket: sorted_unique(ids) for bucket, ids in normal_gear_by_bucket.items()
        },
        "dofusTrophyPrysmaraditeByLevelBucket": {
            bucket: sorted_unique(ids) for bucket, ids in dofus_like_by_bucket.items()
        },
...
def build_index(source: str = "db") -> dict[str, Any]:
    ...
    payload = {
        ...
        "items": [
            ...
            for item in sorted(items, key=lambda item: str(item.get("dofusID", "")))
        ],
        "sets": {
            set_id: serializable_set(set_obj)
            for set_id, set_obj in sorted(sets.items())
        },
    ...
    payload["datasetVersion"] = f"sha256:{digest}"
    return payload
...
def default_output_path() -> str:
    app_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    return os.path.join(
        app_root, "app", "database", "data", "build_discovery_index.json"
    )
