"""Condition evaluation for Build Discovery.
...
def condition_can_pass_with_stat_bounds(
    condition_obj: dict[str, Any],
    stat_bounds: dict[str, tuple[int, int]],
) -> bool:
    """Conservatively reject conditions impossible within final-stat bounds."""
    if not condition_obj:
        return True
    if is_leaf_condition(condition_obj):
        stat_name = CONDITION_STAT_TO_STAT_NAME.get(condition_obj["stat"])
        if not stat_name or stat_name not in stat_bounds:
            return True
        minimum, maximum = stat_bounds[stat_name]
        operator = condition_obj["operator"]
    ...
    if condition_obj.get("and"):
        return all(
            condition_can_pass_with_stat_bounds(child, stat_bounds)
            for child in condition_obj["and"]
        )
    if condition_obj.get("or"):
        return any(
